...
# Track 302_4 through 302_11 as eight independent tracking agents, then run
# the existing robot-count-independent verification and DPGO stages.
...
DPVO_DATA_ROOT="${DPVO_IPHONE_ROOT:-/data3/dpvo_cbs_ws/data/iphone_302_4_11_eight_robot}"
DPVO_RESULTS="${DPVO_RESULTS_ROOT:-/data3/dpvo_cbs_ws/results}"
DPVO_RESULT_TAG="${DPVO_OUTPUT_TAG:-iphone_302_4_5_6_7_8_9_10_11_eight_robot_staged_20260831}"
...
export CUDA_MPS_PIPE_DIRECTORY="${CUDA_MPS_PIPE_DIRECTORY:-/tmp/dpvo-iphone-eight-robot-mps-pipe}"
export CUDA_MPS_LOG_DIRECTORY="${CUDA_MPS_LOG_DIRECTORY:-/tmp/dpvo-iphone-eight-robot-mps-log}"
...
run_stage1() {
  echo "[stage1] Tracking eight videos; inter-robot retrieval/GV/PGO are disabled"
  export CUDA_MPS_ACTIVE_THREAD_PERCENTAGE="${DPVO_TRACKING_MPS_PERCENTAGE:-12}"
  ...
  ros2 launch dpvo_multi_robot eight_robot_tum_track.launch.py \
    ...
    sequence5:="${DPVO_IPHONE_SEQUENCE5:-302_9}" \
    sequence6:="${DPVO_IPHONE_SEQUENCE6:-302_10}" \
    sequence7:="${DPVO_IPHONE_SEQUENCE7:-302_11}" \
