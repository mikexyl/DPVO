...
# Track 302_4 through 302_9 as six independent tracking agents, then run the
# existing robot-count-independent verification and DPGO stages.
...
DPVO_DATA_ROOT="${DPVO_IPHONE_ROOT:-/data3/dpvo_cbs_ws/data/iphone_302_4_9_six_robot}"
DPVO_RESULTS="${DPVO_RESULTS_ROOT:-/data3/dpvo_cbs_ws/results}"
DPVO_RESULT_TAG="${DPVO_OUTPUT_TAG:-iphone_302_4_5_6_7_8_9_six_robot_staged_20260831}"
...
export CUDA_MPS_PIPE_DIRECTORY="${CUDA_MPS_PIPE_DIRECTORY:-/tmp/dpvo-iphone-six-robot-mps-pipe}"
export CUDA_MPS_LOG_DIRECTORY="${CUDA_MPS_LOG_DIRECTORY:-/tmp/dpvo-iphone-six-robot-mps-log}"
...
run_stage1() {
  echo "[stage1] Tracking six videos; inter-robot retrieval/GV/PGO are disabled"
  export CUDA_MPS_ACTIVE_THREAD_PERCENTAGE="${DPVO_TRACKING_MPS_PERCENTAGE:-16}"
  ...
  ros2 launch dpvo_multi_robot six_robot_tum_track.launch.py \
