...
# Run 302_4 through 302_7 as four independent tracking agents, then perform
# offline top-1 loop verification and DPGO from the resulting raw graph.
...
DPVO_DATA_ROOT="${DPVO_IPHONE_ROOT:-/data3/dpvo_cbs_ws/data/iphone_302_four_robot}"
DPVO_RESULTS="${DPVO_RESULTS_ROOT:-/data3/dpvo_cbs_ws/results}"
DPVO_RESULT_TAG="${DPVO_OUTPUT_TAG:-iphone_302_4_5_6_7_four_robot_staged_20260831}"
...
export CUDA_MPS_PIPE_DIRECTORY="${CUDA_MPS_PIPE_DIRECTORY:-/tmp/dpvo-iphone-four-robot-mps-pipe}"
export CUDA_MPS_LOG_DIRECTORY="${CUDA_MPS_LOG_DIRECTORY:-/tmp/dpvo-iphone-four-robot-mps-log}"
...
run_stage1() {
  echo "[stage1] Tracking four videos; inter-robot retrieval/GV/PGO are disabled"
  export CUDA_MPS_ACTIVE_THREAD_PERCENTAGE="${DPVO_TRACKING_MPS_PERCENTAGE:-25}"
  ...
  ros2 launch dpvo_multi_robot four_robot_tum_track.launch.py \
