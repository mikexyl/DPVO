...
# Run one stage at a time, or all three in order:
#   run_three_robot_iphone_staged.sh stage1  # DPVO tracking only
#   run_three_robot_iphone_staged.sh stage2  # BoW top-1 + LightGlue + TEASER++
#   run_three_robot_iphone_staged.sh stage3  # CBS + centralized PGO
#   run_three_robot_iphone_staged.sh all
...
DPVO_RESULT_TAG="${DPVO_OUTPUT_TAG:-iphone_302_1_2_3_staged_20260831}"
...
# A private per-user MPS server avoids the stale system-wide MPS socket on the
# workstation. Three tracking clients get equal compute shares; the sequential
# GV client uses the whole GPU.
export CUDA_MPS_PIPE_DIRECTORY="${CUDA_MPS_PIPE_DIRECTORY:-/tmp/dpvo-iphone-staged-mps-pipe}"
export CUDA_MPS_LOG_DIRECTORY="${CUDA_MPS_LOG_DIRECTORY:-/tmp/dpvo-iphone-staged-mps-log}"
...
run_stage1() {
  echo "[stage1] Tracking three videos; inter-robot retrieval/GV/PGO are disabled"
  export CUDA_MPS_ACTIVE_THREAD_PERCENTAGE="${DPVO_TRACKING_MPS_PERCENTAGE:-33}"
  ...
  ros2 launch dpvo_multi_robot three_robot_tum_track.launch.py \
