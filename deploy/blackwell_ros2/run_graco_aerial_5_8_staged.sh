...
SEQUENCES=(aerial-05-40m aerial-06-20m aerial-07-25m aerial-08-25m)
EXPECTED_FRAMES=(1482 1646 1972 1391)
IMAGE_SCALE=0.5
FRAME_STRIDE=4

DPVO_DEPLOY_ROOT="${DPVO_ROOT:-/data3/dpvo_cbs_ws/src/DPVO}"
SOURCE_ROOT="${DPVO_GRACO_SOURCE_ROOT:-/data3/graco}"
RUN_DIR="${DPVO_GRACO_RESULT_ROOT:-/data3/mikexyl/results/dpvo_multi_robot/graco_aerial_5_8_staged_20260902}"
...
CALIBRATION_ROOT="$SOURCE_ROOT/aerial-calibration"
STEREO_CALIBRATION="$CALIBRATION_ROOT/stereo.yaml"
STEREO_IMU_CALIBRATION="$CALIBRATION_ROOT/stereo-imu.yaml"
GROUNDTRUTH_CALIBRATION="$CALIBRATION_ROOT/groundtruth.yaml"
ARTIFACT_ROOT="$RUN_DIR/tracking"
RRD_ROOT="$ARTIFACT_ROOT/rrd"
GV_DIR="$RUN_DIR/geometric_verification/four"
DPGO_DIR="$RUN_DIR/dpgo/four"
EVO_DIR="$RUN_DIR/evo/four"
PLOTS_DIR="$RUN_DIR/plots/four"
STAGE1_GATE="$ARTIFACT_ROOT/stage1_gate_four.json"
GRAPH_GATE="$GV_DIR/graph_gate.json"
METRICS="$EVO_DIR/metrics.json"
FINAL_GATE="$EVO_DIR/expansion_gate.json"
PREFLIGHT_GATE="$RUN_DIR/preflight/aerial-05-stride4-scale0p5-input-rate.json"
ROS_LOG_DIR="${ROS_LOG_DIR:-$RUN_DIR/ros_log}"
GATE_HELPER="$DPVO_DEPLOY_ROOT/deploy/blackwell_ros2/newer_college_staged.py"
MANIFEST_HELPER="$DPVO_DEPLOY_ROOT/deploy/blackwell_ros2/graco_staged.py"
...
for sequence in "${SEQUENCES[@]}"; do
  BAGS+=("$SOURCE_ROOT/${sequence}_full_ros2")
  SOURCE_BAGS+=("$SOURCE_ROOT/$sequence.bag")
  GROUNDTRUTH+=("$SOURCE_ROOT/$sequence.txt")
done
for path in "$DPVO_VOCAB" "$DPVO_DEPLOY_ROOT/dpvo.pth" \
  "$DPVO_DEPLOY_ROOT/config/default.yaml" "$STEREO_CALIBRATION" \
  "$STEREO_IMU_CALIBRATION" "$GROUNDTRUTH_CALIBRATION" \
  "${BAGS[@]}" "${SOURCE_BAGS[@]}" "${GROUNDTRUTH[@]}"; do
  if [[ ! -e "$path" ]]; then
...
ensure_mps() {
  ...
  if ! pgrep -u "$(id -u)" -f '^nvidia-cuda-mps-control( |$)' >/dev/null; then
    echo "CUDA MPS control daemon is not running" >&2
    exit 2
  fi
}
...
artifact_ready() {
  ...
  "${PYTHON[@]}" "$GATE_HELPER" artifact-ready \
    --tracking-root "$ARTIFACT_ROOT" --robot-id "$robot_id" \
    --expected-frames "${EXPECTED_FRAMES[$index]}" --tolerance 0 \
    --rrd "$RRD_ROOT/$robot_id.rrd"
}

record_manifest() {
  local args=(manifest --run-dir "$RUN_DIR" --vocabulary "$DPVO_VOCAB"
    --network "$DPVO_DEPLOY_ROOT/dpvo.pth"
    --config "$DPVO_DEPLOY_ROOT/config/default.yaml"
    --stereo-calibration "$STEREO_CALIBRATION"
    --stereo-imu-calibration "$STEREO_IMU_CALIBRATION"
    --groundtruth-calibration "$GROUNDTRUTH_CALIBRATION")
  for index in "${!ROBOT_IDS[@]}"; do
    args+=(
      --source-bag "${ROBOT_IDS[$index]}=${SOURCE_BAGS[$index]}"
  ...
  local parameters=(
    ...
    "tracking.source_rate_hz=20.0"
    "tracking.input_rate_hz=5.0"
    "tracking.target_input_rate_hz_min=3.0"
    "tracking.target_input_rate_hz_max=5.0"
    "tracking.image_scale=$IMAGE_SCALE"
    "tracking.rectified_source_resolution=[1600,1100]"
    "tracking.dpvo_input_resolution=[800,544]"
    "tracking.random_seed=1234"
    "tracking.start_frame=0"
    "tracking.full_run_max_frames=0"
    "tracking.rectification_alpha=0.0"
    "tracking.camera=/camera_left/image_raw"
    ...
    "evaluation.groundtruth_frame=RTK_Base_ENU_to_cam0"
    "evaluation.transform=T_BASE_CAM0=T_BASE_IMU*T_IMU_CAM0"
    ...
    "runtime.cuda_mps_pipe_directory=$CUDA_MPS_PIPE_DIRECTORY"
    "runtime.cuda_mps_log_directory=$CUDA_MPS_LOG_DIRECTORY"
...
run_preflight() {
  local root="$RUN_DIR/preflight/aerial-05-moving-stride4-scale0p5"
  if [[ ! -f "$root/robot0/manifest.json" ]]; then
    if [[ -d "$root/robot0" ]]; then
      echo "incomplete preflight exists; preserving it: $root/robot0" >&2
      exit 2
    fi
    ros2 launch dpvo_multi_robot single_robot_graco_track.launch.py \
      bag:="${BAGS[0]}" robot_id:=robot0 \
      ...
      stride:="$FRAME_STRIDE" start_frame:=300 max_frames:=300 image_scale:="$IMAGE_SCALE" \
      rectification_alpha:=0.0 random_seed:=1234 local_feature_backend:=disk \
      bow_threshold:=0.01 2>&1 | tee "$RUN_DIR/preflight_aerial05_stride4_scale0p5.log"
  fi
  "${PYTHON[@]}" "$GATE_HELPER" artifact-ready \
    --tracking-root "$root" --robot-id robot0 --expected-frames 300 --tolerance 0
  "${PYTHON[@]}" "$MANIFEST_HELPER" preflight-input-rate \
    --artifact "$root/robot0" --source-rate-hz 20 --stride "$FRAME_STRIDE" \
    --minimum 3 --maximum 5 --output "$PREFLIGHT_GATE"
}
...
track_robot() {
  ...
  ros2 launch dpvo_multi_robot single_robot_graco_track.launch.py \
    bag:="${BAGS[$index]}" robot_id:="$robot_id" \
    network:="$DPVO_DEPLOY_ROOT/dpvo.pth" config:="$DPVO_DEPLOY_ROOT/config/default.yaml" \
    orb_vocab:="$DPVO_VOCAB" tracking_artifact_output:="$ARTIFACT_ROOT" \
    stride:="$FRAME_STRIDE" start_frame:=0 max_frames:=0 image_scale:="$IMAGE_SCALE" rectification_alpha:=0.0 \
    random_seed:=1234 local_feature_backend:=disk bow_threshold:=0.01 \
    rerun_save:="$RRD_ROOT/$robot_id.rrd" \
    rerun_recording_id:="graco-${SEQUENCES[$index]}-$robot_id-tracking" \
...
validate_stage1() {
  local args=(validate-stage1 --tracking-root "$ARTIFACT_ROOT" --rrd-root "$RRD_ROOT"
    --tracking-graph "$ARTIFACT_ROOT/unoptimized_tracking_graph_four.json"
...
run_stage1() {
  ...
  "${PYTHON[@]}" -m dpvo.loop_closure.offline_multirobot assemble \
    ...
    --output-base "$ARTIFACT_ROOT/unoptimized_tracking_graph_four" \
...
run_stage2() {
  ...
  mkdir -p "$GV_DIR"
  nvidia-smi --query-gpu=timestamp,temperature.gpu,power.draw,memory.used,utilization.gpu \
    --format=csv -l 1 >"$RUN_DIR/stage2_gpu_telemetry.csv" 2>&1 &
  local telemetry_pid=$!
  set +e
  "${PYTHON[@]}" -m dpvo.loop_closure.offline_multirobot verify \
    ...
    --random-seed 1234 2>&1 | tee "$RUN_DIR/stage2_four.log"
...
' "$DPGO_DIR/offline_dpgo_provenance.json" || {
  ...
  "${PYTHON[@]}" -m dpvo.loop_closure.offline_dpgo \
    ...
    2>&1 | tee "$RUN_DIR/stage3_four.log"
}
...
final_gate() {
  local args=(final-gate --stage1-gate "$STAGE1_GATE" --graph-gate "$GRAPH_GATE"
    ...
    --plot-prefix graco --output "$FINAL_GATE")
...
run_evaluate() {
  ...
  "${PYTHON[@]}" "$DPVO_DEPLOY_ROOT/deploy/blackwell_ros2/export_graco_evo_tum.py" \
    --graph "$GV_DIR/unoptimized_verified_graph.json" --solver-dir "$DPGO_DIR" \
    --calibration "$STEREO_IMU_CALIBRATION" "${gt_args[@]}" --output-dir "$EVO_DIR"
...
run_plot() {
  final_gate
  if [[ -f "$PLOTS_DIR/graco_trajectories_loops.png" && -f "$PLOTS_DIR/graco_sparse_joint_map_alignment.png" ]]; then
    final_gate plots
    return
  fi
  ...
  "${PYTHON[@]}" "$DPVO_DEPLOY_ROOT/deploy/blackwell_ros2/plot_newer_college.py" \
    ...
    --dataset-label GrAco --output-prefix graco
  local npz_args=()
  ...
  "${PYTHON[@]}" "$DPVO_DEPLOY_ROOT/deploy/blackwell_ros2/visualize_cbs_joint_map.py" \
    ...
    --recording-id "graco-four-cbs-sparse-map" --host-distance-quantile 0.90 \
    --warp-mode posewise
  "${PYTHON[@]}" "$DPVO_DEPLOY_ROOT/deploy/blackwell_ros2/visualize_cbs_joint_map.py" \
    "${npz_args[@]}" --input-graph "$GV_DIR/unoptimized_verified_graph.json" \
    ...
    --recording-id "graco-four-centralized-sparse-map" --host-distance-quantile 0.90 \
    --warp-mode posewise
  "${PYTHON[@]}" "$DPVO_DEPLOY_ROOT/deploy/blackwell_ros2/plot_joint_map_ply.py" \
    ...
    --prefix graco_sparse --projection-axes xy --max-time-difference 0.055
  final_gate plots
}
...
echo "Completed GrAco Aerial 5--8 $STAGE: $RUN_DIR"
