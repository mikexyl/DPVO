...
DPVO_DATA_ROOT="${DPVO_ICL_NUIM_ROOT:-/data3/mikexyl/datasets/icl_nuim}"
...
DPVO_RESULT_TAG="${DPVO_OUTPUT_TAG:-icl_nuim_lr0_lr1_full}"
...
exec ros2 launch dpvo_multi_robot two_robot_tum.launch.py \
  dataset_root:="$DPVO_DATA_ROOT" \
  sequence0:="${DPVO_SEQUENCE0:-living_room_traj0_frei_png}" \
  sequence1:="${DPVO_SEQUENCE1:-living_room_traj1_frei_png}" \
  ...
  bow_nms_radius:="${DPVO_BOW_NMS_RADIUS:-30}" \
