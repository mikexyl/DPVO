...
# Reuse the validated 302_4 through 302_12 tracking artifacts, track 302_13 as
# robot9, then run the robot-count-independent verification and DPGO stages.
...
DPVO_DATA_ROOT="${DPVO_IPHONE_ROOT:-/data3/dpvo_cbs_ws/data/iphone_302_4_13_ten_robot}"
DPVO_RESULTS="${DPVO_RESULTS_ROOT:-/data3/dpvo_cbs_ws/results}"
DPVO_RESULT_TAG="${DPVO_OUTPUT_TAG:-iphone_302_4_5_6_7_8_9_10_11_12_13_ten_robot_staged_20260831}"
DPVO_REUSE_ARTIFACT_ROOT="${DPVO_REUSE_ARTIFACT_ROOT:-/data3/dpvo_cbs_ws/results/iphone_302_4_5_6_7_8_9_10_11_12_nine_robot_incremental_20260831/tracking}"
...
export CUDA_MPS_PIPE_DIRECTORY="${CUDA_MPS_PIPE_DIRECTORY:-/tmp/dpvo-iphone-ten-robot-mps-pipe}"
export CUDA_MPS_LOG_DIRECTORY="${CUDA_MPS_LOG_DIRECTORY:-/tmp/dpvo-iphone-ten-robot-mps-log}"
...
run_stage1() {
  echo "[stage1] Reusing robots0-8 and tracking 302_13 as robot9"
  mkdir -p "$ARTIFACT_ROOT"
  for robot_id in robot0 robot1 robot2 robot3 robot4 robot5 robot6 robot7 robot8; do
  ...
  ros2 launch dpvo_multi_robot robot9_tum_track.launch.py \
    dataset_root:="$DPVO_DATA_ROOT" \
    sequence0:="${DPVO_IPHONE_SEQUENCE9:-302_13}" \
