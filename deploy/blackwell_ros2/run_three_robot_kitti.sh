...
DPVO_RESULT_TAG="${DPVO_OUTPUT_TAG:-kitti00_three_robot_full}"
...
if [[ -n "${DPVO_RERUN_SAVE_DIR:-}" ]]; then
  ...
  rerun_args+=(
    "rerun_save0:=$DPVO_RERUN_SAVE_DIR/robot0.rrd"
    "rerun_save1:=$DPVO_RERUN_SAVE_DIR/robot1.rrd"
    "rerun_save2:=$DPVO_RERUN_SAVE_DIR/robot2.rrd"
...
exec ros2 launch dpvo_multi_robot three_robot_kitti.launch.py \
  ...
  start0:="${DPVO_START0:-0}" end0:="${DPVO_END0:-1450}" \
  start1:="${DPVO_START1:-1450}" end1:="${DPVO_END1:-3000}" \
  start2:="${DPVO_START2:-3000}" end2:="${DPVO_END2:-4541}" \
  ...
  megaloc_threshold:="${DPVO_MEGALOC_THRESHOLD:-0.20}" \
