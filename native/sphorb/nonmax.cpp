...
{	
	...
	if (num_corners && !scores) throw std::bad_alloc();
	int pixel[18];
...
void sfastNonmaxSuppression(const xy* corners, const int* scores, int num_corners, vector<KeyPoint>& kps, int partIndex)
{
    ...
    if (!row_start) throw std::bad_alloc();
	...
	for(i=0; i < sz; i++)
	{
		...
		if(pos.y != 0 && row_start[pos.y - 1] != -1) 
		{
			...
			/*Make point_above point to the first of the pixels above the current point,
			  if it exists.*/
			for(; point_above < sz && corners[point_above].y < pos.y && corners[point_above].x < pos.x; point_above++)
			{}
			
			
			for(j=point_above; j < sz && corners[j].y < pos.y && corners[j].x <= pos.x + 1; j++)
			{
				int x = corners[j].x;
