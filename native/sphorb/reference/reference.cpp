...
namespace cv
{
	...
	static void initSORB(const std::string& directory)
	{
		...
		for (int i=0;i<levels;i++)
		{
			...
			snprintf(fileName, MAX_PATH, "%s/geoinfo%d.pfm", directory.c_str(), cells[i]);

			float* geoinfo = new float[(cells[i]+1)*(2*cells[i]+1)*3];
			if (!read_pfm(fileName, geoinfo, (cells[i]+1)*(2*cells[i]+1)*3)) throw std::runtime_error("reference table read failed");
			...
			for (int j=0;j<5;j++)
			{
				snprintf(fileName, MAX_PATH, "%s/imginfo%d_%d.pfm", directory.c_str(), cells[i], j);

				int num = (2*cells[i]+1)*(cells[i]+1)*4;
				num = static_cast<int>(ceil(num / 3.0)) * 3;
				float* partinfo = new float[num];
				if (!read_pfm(fileName, partinfo, num)) throw std::runtime_error("reference table read failed");
				partInfos.push_back(partinfo);
			}
			imgInfos.push_back(partInfos);

			// the mask image
			snprintf(fileName, MAX_PATH, "%s/mask%d.bmp", directory.c_str(), cells[i]);
...
static void splitSphere2(const Mat& im, Mat& oim, int idx, const float* imgInfo)
{
	for(int y=0; y<oim.rows; y++)
	{
		for(int x=0; x<oim.cols; x++)
		{
			...
			uchar v3 = im.at<uchar>(std::min(iy+1,im.rows-1), ix);
			uchar v4 = im.at<uchar>(std::min(iy+1,im.rows-1), (ix+1)%im.cols);
...
static void computeOrbDescriptor(const KeyPoint& kpt, const Mat& img, const Point* pattern, unsigned char* desc, int dsize)
{
...
SPHORB::SPHORB(const std::string& directory, int _nfeatures, int _nlevels, int b): nfeatures(_nfeatures), barrier(b)
{
	initSORB(directory);
	nlevels = std::min(_nlevels, levels);
}
                      ...
                      OutputArray _descriptors, bool useProvidedKeypoints) const
{
	...
	for (int l=0;l<nlevels;l++)
	{
		...
		for (auto& kp:levelKeyPoints) kp.octave=l;
