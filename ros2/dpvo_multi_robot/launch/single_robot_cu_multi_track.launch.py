"""Stage-one DPVO tracking for one CU-Multi RGB ROS2 bag."""
...
def generate_launch_description():
    arguments = [
        ...
        DeclareLaunchArgument("source_robot", default_value="robot1"),
        ...
        parameters=[
            {
                ...
                "source_topic": ParameterValue(
                    [
                        LaunchConfiguration("source_robot"),
                        "/camera/color/image_raw",
                    ],
                    value_type=str,
                ),
                "source_camera_info_topic": ParameterValue(
                    [
                        LaunchConfiguration("source_robot"),
    ...
    stop_after_tracker = RegisterEventHandler(
        OnProcessExit(
            ...
            on_exit=[EmitEvent(event=Shutdown(reason="CU-Multi tracking export complete"))],
        )
    )
