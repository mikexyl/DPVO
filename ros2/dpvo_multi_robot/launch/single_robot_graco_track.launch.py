"""Stage-one DPVO tracking for one GrAco aerial ROS2 bag."""
...
def generate_launch_description():
    ...
    # The existing raw-image player is dataset-agnostic at runtime: it reads
    # explicit Image/CameraInfo topics, rectifies pinhole/radtan images, and
    # waits for one DPVO acknowledgement before reading the next frame.
    player = Node(
        package="dpvo_multi_robot",
        executable="dpvo_cu_multi_player",
        namespace=LaunchConfiguration("robot_id"),
        name="graco_player",
        output="screen",
        parameters=[
            {
                ...
                "source_topic": "/camera_left/image_raw",
                "source_camera_info_topic": "/camera_left/camera_info",
                "image_topic": "camera/image_raw",
                "camera_info_topic": "camera/camera_info",
                "frame_ack_topic": "dpvo/frame_ack",
                "done_topic": "dpvo/player_done",
    ...
    stop_after_tracker = RegisterEventHandler(
        OnProcessExit(
            ...
            on_exit=[EmitEvent(event=Shutdown(reason="GrAco tracking export complete"))],
        )
    )
