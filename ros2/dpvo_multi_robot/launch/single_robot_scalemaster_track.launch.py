"""Stage-one DPVO tracking for one ScaleMaster image sequence."""
...
def generate_launch_description():
    ...
    player = Node(
        package="dpvo_multi_robot",
        executable="dpvo_scalemaster_player",
        namespace=LaunchConfiguration("robot_id"),
        name="scalemaster_player",
        output="screen",
        parameters=[
            {
    ...
    stop_after_tracker = RegisterEventHandler(
        OnProcessExit(
            ...
            on_exit=[
                EmitEvent(
                    event=Shutdown(reason="ScaleMaster tracking export complete")
                )
            ],
        )
    )
